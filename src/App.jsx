...
/*  ADMIN DASH  */
function AdminDash({setTab}){
  const [stats,setStats]=useState({agencies:0,users:0,creators:0,revenue:0});
  const [loading,setLoading]=useState(true);
  
  useEffect(()=>{
    const load=async()=>{
      const [agencies,profiles,creators]=await Promise.all([
        fetchAllAgencies(),
        fetchAllProfiles(),
        fetchAllCreators()
      ]);
      const paying=agencies.filter(a=>a.billing_status==="actif"&&!a.is_offered).length;
      setStats({
  ...
  if(loading)return(
    <div className="fup">
      <div style={{textAlign:"center",padding:40,color:T.sec}}><Spin/> Chargement...</div>
    </div>
  );
  
  return(
    <div className="fup">
      <h1 style={{fontSize:24,fontWeight:800,color:T.tx,marginBottom:8}}>Vue globale</h1>
      <div style={{fontSize:13,color:T.sec,marginBottom:20}}></div>
      
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
        <SC label="Agences" val={stats.agencies} sub="Total" accent={T.acc}/>
        <SC label="Utilisateurs" val={stats.users} sub="Inscrits" accent={T.cy}/>
        <SC label="Créateurs" val={stats.creators} sub="Actifs" accent={T.ok}/>
        <SC label="MRR" val={`${stats.revenue}`} sub="Hors offerts" accent={T.stripe}/>
      </div>
      
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12}}>
        <div className="card" style={{padding:16}}>
          <h2 style={{fontSize:16,fontWeight:700,color:T.tx,marginBottom:12}}>Actions rapides</h2>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            <button className="btn" style={{fontSize:12}} onClick={()=>setTab("agencies")}>Voir les agences</button>
            <button className="btn" style={{fontSize:12}} onClick={()=>setTab("invite_agencies")}>Inviter des agences</button>
            <button className="btn" style={{fontSize:12}} onClick={()=>setTab("billing")}>Facturation</button>
          </div>
        </div>
        
        <div className="card" style={{padding:16}}>
          <h2 style={{fontSize:16,fontWeight:700,color:T.tx,marginBottom:12}}>Statistiques</h2>
          <div style={{fontSize:12,color:T.sec,lineHeight:1.6}}>
            <div>Agences actives : {stats.agencies}</div>
            <div>Utilisateurs totaux : {stats.users}</div>
            <div>Créateurs inscrits : {stats.creators}</div>
            <div>Revenu mensuel : {stats.revenue}</div>
          </div>
        </div>
      </div>
    </div>
